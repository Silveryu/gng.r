...
gng.type.aproximate<- function(ef=100){
  c(3, ef)
}
...
.GNG <- function(x=NULL, labels=c(),
  ...
  else if(type[1] == gng.type.aproximate()[1]){
...
AproximateGNG <- function(x=NULL, labels=c(),
beta=0.99,
alpha=0.5,
max.nodes=1000,
eps.n=0.0006,
...
ef = 102
  ...
  call <- match.call(expand.dots = TRUE)
  gng <- .GNG(x=x, labels=labels, beta=beta, alpha=alpha, max.nodes=max.nodes,
  eps.n=eps.n, eps.w=eps.w, max.edge.age=max.edge.age, type=gng.type.aproximate(ef), train.online=train.online, max.iter=max.iter, dim=dim, min.improvement=min.improvement, lambda=lambda, verbosity=verbosity, seed=seed)
  assign("call", call, gng)
  gng
}
