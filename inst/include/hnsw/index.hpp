...
public:
    ...
    std::vector<search_result_t> search(vector_t &target, size_t nearest_neighbors) const {
        return search(target, nearest_neighbors, 100 + nearest_neighbors);
    }


    std::vector<search_result_t> search(vector_t &target, size_t nearest_neighbors, size_t ef) const {
        if (nodes.empty()) {
            return {};
        }
